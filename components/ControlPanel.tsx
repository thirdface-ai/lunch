      ...
      <div className={`w-full max-w-5xl transition-colors duration-300 border shadow-braun-deep p-1 relative ${isDark ? 'bg-dark-bg border-dark-border shadow-dark-deep' : 'bg-braun-bg border-braun-border shadow-braun-deep'}`}>
        ...
        <div className={`pt-6 pb-6 px-8 flex justify-between items-end border-b transition-colors duration-300 ${isDark ? 'border-dark-border bg-dark-surface' : 'border-braun-border bg-[#F4F4F0]'}`}>
            ...
            <div className="flex flex-col items-end gap-2">
                <button onClick={toggleTheme} className={`flex items-center gap-2 px-2 py-1 rounded-[1px] border transition-colors ${isDark ? 'border-dark-border bg-dark-bg' : 'border-braun-border bg-[#E5E5E0]'}`}>
        ...
        <div className={`p-8 border-b transition-colors duration-300 ${isDark ? 'border-dark-border bg-[#111]' : 'border-braun-border bg-[#F0F0EC]'}`}>
            <div className="flex justify-between items-center mb-2">
                <label className={`font-mono text-[9px] font-bold uppercase tracking-widest ${isDark ? darkMuted : 'text-braun-dark'}`}>Target Vector</label>
                {locating && <span className="font-mono text-[9px] text-braun-orange animate-pulse">TRIANGULATING...</span>}
            </div>
                ...
                <div className="relative flex items-center">
                    <div className={`relative flex-grow h-14 flex items-center px-4 ${isDark ? 'bg-[#050505]' : 'bg-[#2A2A2A]'}`}>
                         <input
                            ...
                            onFocus={() => inputValue && predictions.length > 0 && setShowPredictions(true)}
                        ...
                        />
                         ...
                         {showPredictions && predictions.length > 0 && (
                             <div className={`absolute top-full left-0 right-0 mt-1 border shadow-braun-deep z-50 max-h-64 overflow-y-auto ${isDark ? 'bg-dark-bg border-dark-border' : 'bg-[#F4F4F0] border-braun-border'}`}>
                                 {predictions.map((p) => (
                                     <div 
                                         key={p.id}
                                         onMouseDown={(e) => e.preventDefault()}
                                         onClick={() => handlePredictionSelect(p)}
                                         className={`p-3 border-b cursor-pointer font-mono text-xs truncate transition-colors group/item ${isDark ? 'border-dark-border hover:bg-white/5 text-dark-text' : 'border-braun-border/50 hover:bg-braun-orange/10 text-braun-dark'}`}
                                     >
        ...
        <div className={`grid grid-cols-12 border-b ${isDark ? 'border-dark-border' : 'border-braun-border'}`}>
            ...
            <div className={`col-span-12 md:col-span-4 p-8 flex flex-col justify-start gap-8 ${isDark ? 'bg-[#111]' : 'bg-[#F0F0EC]'}`}>
                ...
                <div>
                    ...
                    <div className={`flex h-12 rounded-sm border p-1 gap-1 ${isDark ? 'bg-dark-surface border-dark-border' : 'bg-[#E5E5E0] border-braun-border'}`}>
                        {Object.values(WalkLimit).map((limit) => (
                            <button
                                key={limit}
                                onClick={() => setPreferences(prev => ({ ...prev, walkLimit: limit }))}
                                className={`flex-1 flex items-center justify-center rounded-[1px] font-mono text-[9px] font-bold uppercase tracking-wide transition-all duration-200 outline-none
                ...
                <div>
                    ...
                    <button
                        ...
                        className={`
                            w-full h-12 border rounded-sm flex items-center justify-center transition-all duration-150 group
                            ${preferences.noCash 
                                ? `${isDark ? 'bg-dark-text text-dark-bg' : 'bg-braun-dark text-white'} shadow-sm` 
                    ...
                    <div className="flex flex-wrap gap-2">
                        {Object.values(DietaryRestriction).map((restriction) => (
                            <button
                                ...
                                className={`px-3 py-2 rounded-[1px] font-mono text-[9px] font-bold uppercase tracking-wide transition-all duration-200 outline-none border
        ...
        <div className={`grid grid-cols-12 ${isDark ? 'bg-dark-bg' : 'bg-[#F4F4F0]'}`}>
            ...
            <div className={`col-span-12 md:col-span-8 p-8 border-r border-b md:border-b-0 flex flex-col justify-center ${isDark ? 'border-dark-border' : 'border-braun-border'}`}>
                ...
                <div className={`flex h-14 rounded-sm border p-1 gap-1 ${isDark ? 'bg-dark-surface border-dark-border' : 'bg-[#E5E5E0] border-braun-border'}`}>
                    
                    {/* Explicit "ANY BUDGET" Option */}
                    <button
                        onClick={() => handlePriceSelect(null)}
                        className={`flex-1 flex flex-col items-center justify-center rounded-[1px] transition-all duration-200 outline-none
            ...
            <div className="col-span-12 md:col-span-4 p-8 flex">
                <button
                    ...
                    className={`
                        w-full h-full min-h-[60px] relative transition-all duration-200 ease-out rounded-sm flex items-center justify-center group overflow-hidden
