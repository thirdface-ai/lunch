...
const TerminalLog: React.FC<TerminalLogProps> = ({ appState, logs, progress = 0, theme }) => {
  ...
  return (
    <div className={`min-h-screen p-3 sm:p-4 transition-colors duration-300 ${isDark ? 'bg-dark-bg' : 'bg-braun-bg'}`}>
      {/* Main Chassis */}
      <div className={`w-full max-w-5xl mx-auto border p-1 relative min-h-[80vh] sm:min-h-[600px] flex flex-col shadow-braun-deep transition-colors duration-300 ${isDark ? 'bg-dark-bg border-dark-border shadow-dark-deep' : 'bg-braun-bg border-braun-border shadow-braun-deep'}`}>
