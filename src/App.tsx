...
const AppContent: React.FC = () => {
  ...
  const handleCalculate = useCallback(async () => {
    if (!preferences.lat || !preferences.lng || !window.google) return;
    ...
    try {
      ...
      const detailPromises = uniquePlaceIds.map(id => {
        ...
        return place.fetchFields({ fields: comprehensiveFields as unknown as string[] });
      });
      ...
      const mappedPlaces: GooglePlace[] = places.map((p) => ({
        place_id: p.id || '',
        name: p.displayName || '',
        rating: p.rating,
        ...
        types: p.types,
        price_level: p.priceLevel !== undefined && p.priceLevel !== null 
          ? (['FREE', 'INEXPENSIVE', 'MODERATE', 'EXPENSIVE', 'VERY_EXPENSIVE'].indexOf(p.priceLevel as unknown as string) + 1) || undefined
          : undefined,
        editorial_summary: p.editorialSummary ? { overview: p.editorialSummary } : undefined,
      ...
      const matrixPromises = batches.map(batch => 
        new Promise<void>((resolve) => {
          matrixService.getDistanceMatrix({
            ...
            destinations: batch.map(p => p.geometry?.location).filter(Boolean) as google.maps.LatLng[],
